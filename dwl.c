...
applybounds (Client *c, struct wlr_box *bbox)
{
	...
	/* Some clients set their max size to INT_MAX, which does not
	 * violate the protocol but its unnecesary, as they can set their
	 * max size to zero.
	 */
...
arrangelayer (Monitor *m, struct wl_list *list, struct wlr_box *usable_area, int exclusive)
{
	...
	wlr_scene_node_set_position (&layersurface->popups->node, layersurface->scene->node.x,
...
buttonpress (struct wl_listener *listener, void *data)
{
	...
	if (!locked && cursor_mode != CurNormal && cursor_mode != CurPressed) {
	    ...
	    /* Clear the pointer focus, this way if the cursor is over
	     * a surface we will send an enter event after which the
	     * client will provide us a cursor surface */
...
checkidleinhibitor (struct wlr_surface *exclude)
{
	...
	if (exclude != surface && (bypass_surface_visibility || (!tree || tree->node.enabled))) {
	    inhibited = 1;
	    break;
	}
    }
...
commitnotify (struct wl_listener *listener, void *data)
{
    ...
    if (c->mon && !wlr_box_empty (&box)
        && (box.width != c->geom.width - 2 * c->bw || box.height != c->geom.height - 2 * c->bw))
...
createlayersurface (struct wl_listener *listener, void *data)
{
    ...
    LISTEN (&wlr_layer_surface->surface->events.commit, &layersurface->surface_commit,
...
{
    /* This event is raised when wlr_xdg_shell receives a new xdg surface from
     * a client, either a toplevel (application window) or popup, or when
     * wlr_layer_shell receives a new popup from a layer. If you want to do
     * something tricky with popups you should check if its parent is
     * wlr_xdg_shell or wlr_layer_shell */
	...
	xdg_surface->surface->data
	    = wlr_scene_xdg_surface_create (xdg_surface->popup->parent->data, xdg_surface);
	if ((l && !l->mon) || (c && !c->mon))
	    return;
...
mapnotify (struct wl_listener *listener, void *data)
{
	...
	/* Ideally we should do this in createnotify{,x11} but at that
	 * moment wlr_xwayland_surface doesn't have wlr_surface yet. */
...
unset_fullscreen:
    ...
    wl_list_for_each (w, &clients, link) if (w != c && w->isfullscreen && m == w->mon
...
motionabsolute (struct wl_listener *listener, void *data)
{
     ...
     * edge, so we have to warp the mouse there. There is also some hardware
     * which emits these events. */
...
moveresize (const Arg *arg)
{
	...
	wlr_xcursor_manager_set_cursor_image (cursor_mgr, (cursor_image = "bottom_right_corner"),
	                                      cursor);
	break;
    }
}
...
outputmgrapplyortest (struct wlr_output_configuration_v1 *config, int test)
{
	...
	if (config_head->state.mode)
	    ...
	    wlr_output_set_custom_mode (wlr_output, config_head->state.custom_mode.width,
...
setmon (Client *c, Monitor *m, unsigned int newtags)
{
	...
	c->tags = newtags ? newtags : m->tagset[m->seltags]; /* assign tags of
	                                                        target monitor */
...
setup (void)
{
     ...
     * if an X11 server is running. The NULL argument here optionally allows
     * you to pass in a custom renderer if wlr_renderer doesn't meet your
     * needs. The backend uses the renderer, for example, to fall back to
     * software cursors if the backend does not support hardware cursors (some
     * older GPUs don't). */
